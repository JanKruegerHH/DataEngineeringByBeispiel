...
def solve_target_optimization(weights, lower_bounds, upper_bounds, target):
    ...
    lb = np.array([L[i] for i in range(n)] + [0.0])
    ub = np.array([min(U[i], 1.0) for i in range(n)] + [np.inf])
...
stations = ["FRA", "MUC", "ZRH"]

weights = [0.5, 0.375, 0.125]  # must sum to 1.0
wop     = [0.8333, 0.6165, 0.44]  # current WOP baseline
lower   = [0.5, 0.5, 0.50]  # L_i
upper   = [0.9, 0.9, 0.9]  # U_i
target  = 0.7          # T

result = solve_target_optimization(weights, lower, upper, target)

# ── Print Results ──────────────────────────────────────────────────────────────
logger.info(f"{'Station':<12} {'Weight':>8} {'WOP':>6} {'Lower':>7} {'Upper':>7} {'Proposal':>10}")
logger.info("-" * 55)
for i, station in enumerate(stations):
    logger.info(f"{station:<12} {weights[i]:>8.3f} {wop[i]:>6.3f} "
